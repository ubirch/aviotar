...
package ubirch.timeseries.components
...
trait ElasticSearchStorageComponent extends StorageComponent[JValue] with LazyLogging {
   ...
   */
  protected def validateTimestamp(v: JValue): JValue = (v \ timestamp).toOpt match {
  ...
  class ElasticSearchStorage(client: CloseableHttpClient, uri: URI) extends Storage[JValue] {
    ...
    override def store(key: String, value: JValue): Future[JValue] = Future {
    ...
    override def delete(key: String): Future[Boolean] = Future {
      ...
      execute(deleteRequest) match {
        case (Some(r), status) if status == 200 => r \ "acknowledged" extractOrElse false
        case (Some(r), status) if status != 404 && status > 300 => throw new Exception((r \ "error").extract[String])
        case _ => false
      }
    }
